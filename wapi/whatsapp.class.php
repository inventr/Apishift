...
class WhatsApp
	...
	*/
		...
		if ($rescount != 0)
			...
			foreach($resarray as $k => $v)
				...
				if ($rcvd_type == 'msg')
					...
					// echo "[] Message from ".$msg['from_number']." '".$msg['message_id']."': ".$msg['body_txt']."\n";

					json_encode($msg); // Do something with the message here ?
					$numero = $msg['from_number'];
					$xml = new DomDocument('1.0', 'UTF-8');
					$root = $xml->createElement('Whatsapp');
					$root = $xml->appendChild($root);
					$message = $xml->createElement('message');
					$message = $root->appendChild($message);
					$from = $xml->createElement('from',$msg['from_number']);
					$from = $message->appendChild($from);
					$id = $xml->createElement('id',$msg['message_id']);
					$id = $message->appendChild($id);
					$body = $xml->createElement('body',$msg['body_txt']);
					$body = $message->appendChild($body);
					$tiempo = $xml->createElement('time',$msg['timestamp']);
					$tiempo = $message->appendChild($tiempo);
					...
					$xml->save('data.xml', LIBXML_NOENT);
	$from = $msg['from_number'];
	$numero = substr($from, 1, 10);
	$cadena = $msg['from_number'];
	$fromsql = "0" .$cadena;
	$to = "5558965";
	$buscar = $numero;
	...
	if ($resultado !== FALSE)
	{
		$conexion = mysql_connect("dbone.gerswin.com", "desdecolon", "x4PsxuyeEHW9crs2");
		mysql_select_db("xsms", $conexion);
		// Revisar Duplicados
		$dup = $msg['timestamp']; 
		$msgql = $msg['body_txt'];
		$consulta = "select * from w where tiempo='$dup' or msg= '$msgql' ";
			
		$resultado = mysql_query($consulta) or die(mysql_error());
		if (mysql_num_rows($resultado) == 0)
		{
			$query = " INSERT INTO w (idw, de, para, tiempo, msg)";
			$query.= " VALUES ('" . $msg['message_id'] . "','" . $fromsql . "','" . $to . "','" . $msg['timestamp'] . "','" . $msg['body_txt'] . "')";
	...
	function Login()
		{
		$Data = "WA" . "\x01\x01\x00\x19\xf8\x05\x01\xa0\x8a\x84\xfc\x11" . "$this->_device-$this->_whatsAppVer-$this->_port" . "\x00\x08\xf8\x02\x96\xf8\x01\xf8\x01\x7e\x00\x07\xf8\x05\x0f\x5a\x2a\xbd\xa7";
		$this->send($Data);
		$Buffer = $this->read();
		...
		$this->read();
		$name = $this->_nickname;
		$next = "\x00" . chr(8 + strlen($name)) . "\xf8\x05\x74\xa2\xa3\x61\xfc" . chr(strlen($name)) . $name . "\x00\x15\xf8\x06\x48\x43\x05\xa2\x3a\xf8\x01\xf8\x04\x7b\xbd\x4d\xf8\x01\xf8\x03\x55\x61\x24" . "\x00\x12\xf8\x08\x48\x43\xfc\x01\x32\xa2\x3a\xa0\x8a\xf8\x01\xf8\x03\x1f\xbd\xb1";
	...
	function _authenticate($nonce, $_NC = '00000001')
		{
		$cnonce = random_uuid();
	...
	function sendImage($msgid, $to, $path, $size, $link, $b64thumb)
		{
	...
	function RequestLastSeen($mobile)
		{
		$mob_len = chr(mb_strlen($mobile, "UTF-8"));
